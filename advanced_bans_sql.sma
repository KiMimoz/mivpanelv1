...
public PrepareTable()
{
	...
	formatex(query, sizeof(query) - 1,\
		"CREATE TABLE IF NOT EXISTS %s (%s varchar(32), %s varchar(35), %s int(10), %s varchar(32), %s varchar(128), %s varchar(64), %s varchar(35));",\
...
public CmdBan(client, level, cid)
{
	...
	PrintActivity(admin_name, "^x04[Advanced Bans] $name^x01 :^x03  banned %s. Reason: %s. Ban Length: %s", target_name, arg, unban_time);
...
public CmdAddBan(client, level, cid)
{
	...
	PrintActivity(admin_name, "^x04[Advanced Bans] $name^x01 :^x03  banned %s %s. Reason: %s. Ban Length: %s", is_ip ? "IP" : "SteamID", target_authid, reason, unban_time);
...
public CmdUnban(client, level, cid)
{
	...
	for( new i = 0; i < g_total_bans; i++ )
	{
		...
		if( equal(arg, banned_authid) )
		{
			...
			PrintActivity(admin_name, "^x04[Advanced Bans] $name^x01 :^x03  unbanned %s^x01 [%s] [Ban Reason: %s]", name, arg, reason);
	...
	if( TrieKeyExists(g_trie, arg) )
	{
		...
		PrintActivity(unban_name, "^x04[Advanced Bans] $name^x01 :^x03  unbanned %s^x01 [%s] [Ban Reason: %s]", data[bd_name], data[bd_steamid], data[bd_reason]);
	...
	#endif
	{
		...
		for( new i = 0; i < g_total_bans; i++ )
		{
			...
			|| u_year == c_year && u_month == c_month && u_day == c_day && u_hours == c_hours && u_minutes == c_minutes && u_seconds <= c_seconds )
			{
				...
				Print("^x04[Advanced Bans]^x03 %s^x01[^x04%s^x01]^x03 ban time is up!^x01 [Ban Reason: %s]", data[bd_name], data[bd_steamid], data[bd_reason]);
				
				RemoveBan(i, data[bd_steamid]);
				#else
				Log("Ban time is up for: %s [%s]", g_names[i], g_steamids[i]);
				
				Print("^x04[Advanced Bans]^x03 %s^x01[^x04%s^x01]^x03 ban time is up!^x01 [Ban Reason: %s]", g_names[i], g_steamids[i], g_reasons[i]);
...
AddBan(const target_name[], const target_steamid[], const reason[], const length, const unban_time[], const admin_name[], const admin_steamid[])
{
	...
	static data[BannedData];
	static query[512];
	if(data[bd_name] || data[bd_steamid]) {
		formatex(query, sizeof(query) - 1,\
			"UPDATE %s SET (%s, %s, %s, %s, %s, %s, %s) VALUES ('%s', '%s', '%i', '%s', '%s', '%s', '%s') WHERE `victim_name` = '%s';",\
			TABLE_NAME, KEY_NAME, KEY_STEAMID, KEY_BANLENGTH, KEY_UNBANTIME, KEY_REASON, KEY_ADMIN_NAME, KEY_ADMIN_STEAMID,\
			target_name2, target_steamid, length, unban_time, reason2, admin_name2, admin_steamid, target_name
			);
	} else {
		formatex(query, sizeof(query) - 1,\
...
RemoveBan(remove)
{
	...
	static query[128];
	formatex(query, sizeof(query) - 1,\
		"UPDATE %s SET `Active` = '0' WHERE %s = '%s';",\
